...
async def play(ctx=SlashContext, *, query=None):
    if not query and ctx.author.voice.is_paused():
        return ctx.author.voice.resume()
    elif not query:
    ...
    # voice = get(bot.author.voices, guild=ctx.guild)
    ...
    if(len(_queue) >= 1):
        ...
        if not voice.is_playing():
            asyncio.run_coroutine_threadsafe(
                ctx.author.voice.disconnect(), bot.loop)
            asyncio.run_coroutine_threadsafe(
